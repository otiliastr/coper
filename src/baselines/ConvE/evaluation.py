...
def ranking_and_hits(model, dev_rank_batcher, vocab, name, sess = None):
    log.info('')
    log.info('-'*50)
    log.info(name)
    log.info('-'*50)
    ...
    for i, str2var in enumerate(dev_rank_batcher):
        ...
        if sess is None:
            ...
            pred1 = sess.run(model.pred, feed_dict = {model.e1: e1, model.rel: rel})
            pred2 = sess.run(model.pred, feed_dict = {model.e1: e2, model.rel: rel_reverse})
            pred1 = torch.from_numpy(pred1)
            pred2 = torch.from_numpy(pred2)
        #pred1, pred2 = pred1.data, pred2.data
        #print("The prediction size of pred1 is: {}".format(pred1))
