...
class BiLinearReg(object):
    def __init__(self, Config):
        ...
        #self.condition = tf.greater_equal(self.reg_sim, self.sim_threshold)
        #self.reg_loss_weight = tf.where(self.condition, tf.ones_like(self.reg_sim)*-1., tf.ones_like(self.reg_sim))

        self.reg_loss_weight = -1. * self.reg_sim 
       ...
       # self.batch_maps = tf.concat(tensor_maps, 0)
        ...
        self.reg_predictions = tf.nn.l2_normalize(self.reg_predictions, 1)
        self.reg_rel_embs = tf.nn.l2_normalize(self.reg_rel_embs, 1)
        ...
        # Uncomment below for Cosine Similarity
        """
        # normalize row bases
        ...
        """
        # Uncomment below for similarity loss maps
         
        self.W_r_weight = self.reg_sim / self.reg_agg_sim
        self.W_r_weight = tf.reshape(self.W_r_weight, [batch_size, 1])
        self.reg_sim_col = tf.reshape(self.reg_sim, [batch_size, 1])
        self.W_r_scaled = self.W_r_weight * self.reg_rel_embs
        self.S_scaled = self.reg_predictions * self.reg_sim_col
        # remove gradient flow
        self.S_scaled = tf.stop_gradient(self.S_scaled)
        self.reg_loss = tf.reduce_sum(tf.square(self.W_r_scaled - self.S_scaled))
