...
def ranking_and_hits(model, results_dir, data_iterator_handle, name, session=None, hits_to_compute=(1, 3, 5, 10, 20),
    ...
    hits_base = {hits_level: [].copy() for hits_level in hits_to_compute}
    hits = copy.deepcopy(hits_base)
    # Computing relationwise metrics
    if get_relation_metrics:
        relation_hits = defaultdict(lambda: copy.deepcopy(hits_base))
    ...
    while not stopped:
        try:
            ...
            for i in range(len(e1)):
                ...
                ranks.append(rank)
                rel_name = id_rel_map[rel[i]]
                if 'ranks' not in relation_hits[rel_name]:
                    relation_hits[rel_name]['ranks'] = []
                relation_hits[rel_name]['ranks'].append(rank)

                for hits_level in hits_to_compute:
                    if rank <= hits_level:
                        hits[hits_level].append(1.0)
                        if get_relation_metrics:
                            relation_hits[rel_name][hits_level].append(1.0)
                    else:
                        hits[hits_level].append(0.0)
                        if get_relation_metrics:
                            relation_hits[rel_name][hits_level].append(0.0)

                    #if len(relation_hits['boo'][hits_level]) > 0: 
                     #   logger.info('Duplicate!')
                      #  logger.info(rel_name)
                       # logger.info('boo after: {}'.format(relation_hits['boo']))
                       # logger.info(relation_hits[rel_name])
                       # print(relation_hits['boo'][hits_level])
                        #exit()
    ...
    if get_relation_metrics and enable_write_to_file:
        ...
        for rel in relation_hits.keys():
            for hits_level in hits_to_compute:
                ...
                _write_data_to_file(relation_save_path, '{}\t{}'.format(rel, str(hits_value)))
            
            if 'ranks' not in relation_hits[rel]:
                logger.info('{} ranks: {}'.format(rel, relation_hits[rel]))
                exit()
            ...
            _write_data_to_file(relation_mr_path, '{}\t{}'.format(rel, str(mr)))
            _write_data_to_file(relation_mrr_path, '{}\t{}'.format(rel, str(mrr)))
