...
from model import ConvE, DistMult, Complex, ConvE_tf
...
def main():
    ...
    if Config.model_name is None:
        ...
        model = ConvE(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'ConvE_tf':
        model = ConvE_tf(vocab['e1'].num_token, vocab['rel'].num_token)
    ...
    if Config.model_name != 'ConvE_tf' and Config.cuda:
       model.cuda()
    if load:
    ...
    if Config.model_name != 'ConvE_tf':
        ...
        # copied from: https://stackoverflow.com/questions/38160940/how-to-count-total-number-of-trainable-parameters-in-a-tensorflow-model
        total_parameters = 0
    ...
    for epoch in range(epochs):
        ...
        for i, str2var in enumerate(train_batcher):
            ...
            if Config.model_name != 'ConvE_tf':
                ...
                sess.run(model.train_op, feed_dict = {model.e1: e1, model.rel: rel, model.e2_multi: e2_multi})
                loss = sess.run(model.loss, feed_dict = {model.e1: e1, model.rel: rel, model.e2_multi: e2_multi})
        ...
        if Config.model_name != 'ConvE_tf':
            model.eval()
            with torch.no_grad():
                ranking_and_hits(model, dev_rank_batcher, vocab, 'dev_evaluation', None)
                if epoch % 3 == 0:
                    if epoch > 0:
                        ranking_and_hits(model, test_rank_batcher, vocab, 'test_evaluation', None)
        else:
            ranking_and_hits(model, dev_rank_batcher, vocab, 'dev_evaluation', sess)
            if epoch % 3 == 0:
                if epoch > 0:
                    ranking_and_hits(model, test_rank_batcher, vocab, 'test_evaluation', sess)
