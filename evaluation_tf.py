...
# taken from https://stackoverflow.com/questions/600268/mkdir-p-functionality-in-python
def mkdir_p(path):
    try:
        os.makedirs(path)
...
def ranking_and_hits(model, model_name, dev_rank_batcher, vocab, name, sess=None):
    ...
    for i in range(10):
        ...
        write_data_to_file(hits_right_path, np.mean(hits_left[i]))
        write_data_to_file(hits_left_path, np.mean(hits_right[i]))
        write_data_to_file(hits_at_path, np.mean(hits[i]))
    ...
    write_data_to_file(mean_rank_left, np.mean(ranks_left))
    write_data_to_file(mean_rank_right, np.mean(ranks_right))
    write_data_to_file(mean_rank, np.mean(ranks))
    write_data_to_file(mrr_right, np.mean(1. / np.array(ranks_right)))
    write_data_to_file(mrr_left, np.mean(1. / np.array(ranks_left)))
    write_data_to_file(mrr, np.mean(1. / np.array(ranks)))
